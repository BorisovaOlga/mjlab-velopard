...
| `clock_gallop` | 1.5 | награда | Совпадение контактов с фазовым шаблоном галопа |
...
### `clock_gallop` — фазовый шаблон галопа
...
Период равен `0.4 с`. Для порядка каналов `FL, FR, RL, RR` используются offsets
`(0.0, 0.5, 0.25, 0.75)`, что задаёт начала опорных фаз в порядке:

```text
FL → RR → FR → RL
```

Каждая опорная фаза занимает `20%` цикла. Reward равен доле лап, у которых
фактическое состояние контакта совпало с требуемым. Метрика:
`Metrics/clock_gait_agreement`.
...
Компонент включается выше `1.8 м/с`. В отличие от `spine_flexion`, который
